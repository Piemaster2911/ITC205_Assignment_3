...
public class TestScenarioBookOnLoan {
    ...
    @Test
    public void testMemberBorrowBookLoanErrorOnScanLoanedBook() {
        int memberID = 3;
        int[] bookIDs = {2, 5, 9, 10};
        
        List<ILoan> prevList = loanDAO.listLoans();
        ...
        System.out.println("\n\nThis test will perform a scenario where member " 
                ...
                + "\nto be processed and conclude the transaction. However, she will scan"
                + "\none book that is mistakenly set to be already on loan.\n"
        ...
        for(int i = 0; i < bookIDs.length; i++) {
            ctl.bookScanned(bookIDs[i]);
        }
        
        System.out.println("\nAll books have been scanned!");
        ctl.scansCompleted();
        System.out.println("\nScanning completed! Borrow books?");
        ctl.loansConfirmed();
        System.out.println("\nLoan confirmed! Thank you for your service, " + memberName + ".");
        ...
        List<ILoan> tempList = memberDAO.getMemberByID(memberID).getLoans();
        ...
        boolean hasInvalidBook = false;
        
        for(int i = 0; i < tempList.size(); i++) {
            for(int k = 0; k < prevList.size(); k++) {
                if(tempList.get(i).equals(prevList.get(k)))
                    hasInvalidBook = true;
            }
        }
        ...
        if(hasInvalidBook) {
            testValid = false;
            System.out.println("\nMember " + memberName + " has invalid loans! -- FAIL");
        }
        else
            System.out.println("\nMember " + memberName + " has loans confirmed and on loan! -- PASS");
        
        Assert.assertTrue(testValid);
    }
